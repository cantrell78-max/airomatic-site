...
### 1. The yacht won’t let you ghost the check

On **Diligence**, the money conversation is a real quest marker:

- Clear path upstairs to talk terms with Garry  
- Party / canapé routes still work — but you **can’t leave the boat** until you **take the seed**, negotiate a **SAFE**, or **decline** on purpose  
- Fox onesie (Dylan) still unlocks Oakland; he no longer replaces the term sheet  

So Chapter 1 still ends in free roam — but almost every run now has a **capital decision** on the record (`raisedSeed` or `declinedSeed`).

| Choice | Rough cash | Flag energy |
|--------|------------|-------------|
| Take the seed | +$150k | Baller runway |
...
Mini-arc (cash sinks, comedy, progress):

1. **Lease** the glass box ($14k–$18k)  
2. **Furnish** — frugal, Series-A cosplay, or stolen lounge chairs  
3. **First hire** — Avery (Chief of Staff, 23, already dead inside), the roommate (DAO the org chart), or solo founder mode  
...
- Green-room scene at HQ  
- Optional public “exploring a Series A” post (or stealth raise)  
- Hub for roaming while LPs circle  

**Still to write** (parked on purpose): full Spaces boss fight, flaky term sheets, data-room dungeon, board with Mom on Zoom, USB mid-diligence. The unlock is live so the seed path has a destination.
